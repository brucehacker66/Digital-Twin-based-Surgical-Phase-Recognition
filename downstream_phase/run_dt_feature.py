...
sys.path.append("/mnt/disk0/haoding/Surgformer")
...
from downstream_phase.datasets_phase_dt_feature import build_dataset_dt_feature
from downstream_phase.engine_for_phase_dt_feature import (
    train_one_epoch_feature,
    validation_one_epoch_feature,
    final_phase_test_feature,
    merge_feature,
)
...
from model.surgformer_HTA_KCA_dt import surgformer_HTA_KCA_dt
from model.surgformer_HTA_KCA_dt_feature import surgformer_HTA_KCA_dt_feature
...
def get_args():
    ...
    parser.add_argument(
        "--pretrained_path",
        default="/home/haoding/Bruce/Surgformer/pretrain_params/TimeSformer_divST_8x32_224_K400.pyth",
        type=str,
        metavar="Parameters",
        help="Name of parameters to load",
    )
    parser.add_argument(
        "--use_pretrain", action="store_true", default=False
    )
...
def main(args, ds_init):
    ...
    dataset_train, args.nb_classes = build_dataset_dt_feature(
    ...
    if args.eval:
        ...
        test_stats = final_phase_test_feature(data_loader_test, model, device, preds_file)
        print("Save Files: ", preds_file)
        #torch.distributed.barrier()
        if global_rank == 0:
            print("Start merging results...")
            final_top1, final_top5 = merge_feature(args.output_dir, num_tasks)
            print(
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch_feature(
            model,
            criterion,
            data_loader_train,
            optimizer,
        ...
        if False:
            test_stats = validation_one_epoch_feature(data_loader_val, model, device)
            print(
