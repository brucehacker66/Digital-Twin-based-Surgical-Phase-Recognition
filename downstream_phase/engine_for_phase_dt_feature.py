...
def train_class_batch(model, samples, target, criterion, features=None):
...
def train_one_epoch_feature(
    ...
    # samples match to the buffer in Cholec80 dataloader
    for data_iter_step, (samples, targets, features, _, _) in enumerate(
        ...
        if loss_scaler is None:
            ...
            loss, output = train_class_batch(model, samples, targets, criterion, features)
        else:
...
@torch.no_grad()
def validation_one_epoch_feature(data_loader, model, device):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        # print(f"Input data type: {videos.dtype}")
        # print(f"Target shape: {target.shape}")
        # print(f"Target data type: {target.dtype}")
        # print(f"Min/Max of input data: {videos.min().item()}/{videos.max().item()}")

        # compute output
        with torch.cuda.amp.autocast():
            output = model(videos, features)
...
@torch.no_grad()
def final_phase_test_feature(data_loader, model, device, file):
    criterion = torch.nn.CrossEntropyLoss()
...
def merge_feature(eval_path, num_tasks):
    dict_feats = {}
    dict_label = {}
