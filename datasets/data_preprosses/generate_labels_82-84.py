...
def main():
    ROOT_DIR = "/mnt/disk0/haoding/cholec80_original"
    ANNO_DIR = "/mnt/disk0/haoding/cholec80"
    ...
    VIDEO_NAMES = ["video81.mp4", "video82.mp4", "video83.mp4", "video84.mp4"]
    # TEST_NUMBERS = np.arange(11,21).tolist()
    TRAIN_NUMBERS = np.array([])
    TEST_NUMBERS = np.array([81, 82, 83, 84])
    frames_count = [772, 69, 120, 96]
    count = 0
    ...
    for video_name in VIDEO_NAMES:
        ...
        frames = frames_count[count]
        count += 1
        fps = 1

        # 打开Label文件
        tool_path = os.path.join(ANNO_DIR, 'tool_annotations', video_name.replace('.mp4', '-tool.txt'))
        ...
        phase_path = os.path.join(ANNO_DIR, 'phase_annotations', video_name.replace('.mp4', '-phase.txt'))
        ...
        for frame_id in tqdm(range(0, int(frames))):
            if frame_id % fps == 0:
                ...
                assert int(phase[0]) == frame_id * 25
    ...
    # train_save_dir = os.path.join(ROOT_DIR, 'labels', 'train_20')
    # os.makedirs(train_save_dir, exist_ok=True)
    # with open(os.path.join(train_save_dir, '1fpstrain.pickle'), 'wb') as file:
    #     pickle.dump(train_pkl, file)

    test_save_dir = os.path.join(ROOT_DIR, 'labels', 'test_81+')
