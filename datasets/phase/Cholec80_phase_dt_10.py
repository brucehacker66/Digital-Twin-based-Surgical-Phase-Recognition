...
def convert_masks_depth(mask_path, depth_path, frame_id):
    ...
    # mask_file_name = f"{target_frame_id:05d}.png"
    ...
    if os.path.exists(mask_file_path):
        ...
        if not np.all((mask >= 0) & (mask <= 10)):
            raise ValueError(f"Mask file {mask_file_name} contains invalid class values. Expected values between 1 and 10.")
        ...
        new_mask_channels = np.zeros((height, width, 11), dtype=np.float32)
        new_mask_channels[np.arange(height)[:, None], np.arange(width), mask] = 1
        mask_channels = new_mask_channels[:,:,1:]

        # mask_channels now contains only the 10 mask channels without depth
        final_tensor = mask_channels  # Rename for clarity if needed

        # Optional: Debugging to check the tensor shape
        ...
        raise FileNotFoundError(f"Depth file {mask_file_name} not found in {mask_path}")
...
class PhaseDataset_Cholec80_dt_10(Dataset):
        ...
        if mode == "train":
            ...
            self.data_transform = video_transforms.Compose(
                [
                    ...
                    volume_transforms.ClipToTensor_dt_10(),
...
def build_dataset(is_train, test_mode, fps, args):
    if args.data_set == "Cholec80":
        ...
        dataset = PhaseDataset_Cholec80_dt_10(
            anno_path=anno_path,
