...
def convert_masks_depth(mask_path, depth_path, frame_id):
    ...
    if os.path.exists(depth_file_path):
        # Read the depth from png and convert back to npy
        depth = np.array(Image.open(depth_file_path)).astype(np.float32)
        #print("load depth", time.time() - tik)

        # Normalize depth to range [0, 1] while retaining original floating-point precision
        depth = depth / 65535.0
        ...
        #print("load mask", time.time() - tik)

        # Check if mask values are within the range [1, 10]
        if not np.all((mask >= 0) & (mask <= 10)):
            raise ValueError(f"Mask file {mask_file} contains invalid class values. Expected values between 1 and 10.")

        # Convert mask to 10 channels
        # mask_channels = np.zeros((depth.shape[0], depth.shape[1], 10), dtype=np.float32)
        # for c in range(1, 11):
        #     mask_channels[:, :, c - 1] = (mask == c).astype(np.float32)
        
        height, width = mask.shape
        new_mask_channels = np.zeros((depth.shape[0], depth.shape[1], 11), dtype=np.float32)
        new_mask_channels[np.arange(height)[:, None], np.arange(width), mask] = 1
        mask_channels = new_mask_channels[:,:,1:]
        
        #print(((new_mask_channels[:,:,1:] - mask_channels) ** 2).sum())

        # Concatenate depth as the 11th channel
        depth_channel = np.expand_dims(depth, axis=-1)
        final_tensor = np.concatenate((mask_channels, depth_channel), axis=-1)
        
        # print("process mask", time.time() - tik)
        
        return np.array(final_tensor)
    else:
        raise FileNotFoundError(f"Depth file {depth_file_name} not found in {depth_path}")
...
class PhaseDataset_Cholec80_dt_feature(Dataset):
        ...
        if mode == "train":
            ...
            self.data_transform = video_transforms.Compose(
                [
                    ...
                    volume_transforms.ClipToTensor_dt(),
    ...
    def __getitem__(self, index):
        if self.mode == "train":
            ...
            if self.data_strategy == "online":
                buffer, phase_labels, sampled_list, features = self._video_batch_loader(
            ...
            if self.output_mode == "key_frame":
                if self.data_strategy == "offline":
                    return (
                        ...
                        phase_labels[self.clip_len // 2],
                        features,
                        str(index) + "_" + video_id + "_" + stxr(frame_id),
                        flag,
                    )
                elif self.data_strategy == "online":
                    return (
    ...
    def _make_dataset(self, infos):
        frames = []
        for video_id, data in infos.items():
            for line_info in data:
                ...
                img_path = os.path.join(
                    ...
                    "frames",
                    line_info["video_id"],
                    str(int(line_info["frame_id"])).zfill(5) + ".png",
                )
    ...
    def _video_batch_loader(self, duration, indice, video_id, index, cut_black):
        ...
        sampled_image_list = []
        sampled_label_list = []
        # Initialize lists for sampled images and labels
        sampled_image_list = []
        sampled_label_list = []
        ...
        for num, image_index in enumerate(sampled_list):
            ...
            try:
                target_frame_path = self.dataset_samples[image_index]["img_path"]
                real_frame_id = int(os.path.basename(target_frame_path).split(".")[0])
                base_path = os.path.join(*target_frame_path.split(os.sep)[:5])
                
                # change based on path
                mask_path = os.path.join("/" + base_path, "masks", video_id)
                depth_path = os.path.join("/" + base_path, "depths", video_id)
                feature_path = os.path.join("/" + base_path, "features", video_id)
                
                npy_data = convert_masks_depth(mask_path, depth_path, real_frame_id)
                # print(f"Shape of npy_data: {npy_data.shape}")
                # print(f"Type of npy_data: {type(npy_data)}, dtype of npy_data: {npy_data.dtype}")
                
                # npy_feature = np.load(os.path.join(feature_path, f"{real_frame_id*25:07d}.npy"))
                npy_feature = np.load(os.path.join(feature_path, f"{real_frame_id:05d}.npy"))
                
                sampled_image_list.append(npy_data)  # (H, W, 11)
                features_list.append(npy_feature)
        ...
        return video_data, phase_data, sampled_list, features

    # smaple some frames before and after the key frame
    def _video_batch_loader_for_key_frames(self, duration, timestamp, video_id, index, cut_black):
        ...
        for num, image_index in enumerate(sampled_list):
            try:
                ...
                npy_feature = np.load(os.path.join(feature_path, f"{real_frame_id:05d}.npy"))
                npy_data = convert_masks_depth(mask_path, depth_path, real_frame_id)
                sampled_image_list.append(npy_data)
                features_list.append(npy_feature)
