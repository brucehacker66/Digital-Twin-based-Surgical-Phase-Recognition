...
class PhaseDataset_Cholec80_1mask_2depth(Dataset):
    ...
    def _video_batch_loader(self, duration, indice, video_id, index, cut_black):
        ...
        for num, image_index in enumerate(sampled_list):
            ...
            try:
                ...
                # target_frame_id = real_frame_id * 25
                # depth_file_name = f"{target_frame_id:07d}.png"
                depth_file_name = f"{real_frame_id:05d}.png"
                
                mask_path = os.path.join("/" + base_path, "masks", video_id, depth_file_name)
                depth_path = os.path.join("/" + base_path, "depths", video_id, depth_file_name)
                depth_image = Image.open(depth_path)

                mask = np.array(Image.open(mask_path))
                if not np.all((mask >= 0) & (mask <= 10)):
                    raise ValueError(f"Mask file contains invalid class values. Expected values between 1 and 10.")

                mask = mask * 25
                mask_3ch = np.zeros((mask.shape[0], mask.shape[1], 3), dtype=np.uint8)
                mask_3ch[..., 0] = mask

                # Convert the image to a numpy array for manipulation
                depth_np = np.array(depth_image).astype(np.float32)
                depth_normalized = (depth_np / depth_np.max()) * 255.0
                depth_normalized = depth_normalized.astype(np.uint8)

                # Expand depth to 3 channels
                mask_3ch[..., 1] = depth_normalized
                mask_3ch[..., 2] = depth_normalized

                depth_rgb_image = Image.fromarray(mask_3ch)

                phase_label = self.dataset_samples[image_index]["phase_gt"]

                sampled_image_list.append(depth_rgb_image)
...
def build_dataset(is_train, test_mode, fps, args):
    if args.data_set == "Cholec80":
        ...
        dataset = PhaseDataset_Cholec80_3depth(
            anno_path=anno_path,
