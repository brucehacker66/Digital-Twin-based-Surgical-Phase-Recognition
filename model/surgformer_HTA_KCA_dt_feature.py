...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(f"Input dtype before proj: {x.dtype}, shape: {x.shape}")
        # print(f"Projection layer weight dtype: {self.proj.weight.dtype}")
        if x.dtype != torch.float16:  # convert to weight type
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        # add channel reduction
        # self.conv1x1 = nn.Conv2d(in_channels=in_chans, out_channels=3, kernel_size=1)
        ...
        self.head = (
            ...
            # use 2816 if concatenated with encoded features
            nn.Linear(2816, num_classes) if num_classes > 0 else nn.Identity()
        )
        ...
        # layers to convert to [B, 768]
        self.flatten_layer = nn.Flatten(start_dim=2)  # Flatten all dimensions except batch
        self.fc1 = nn.Linear(2 * 1024, 1024)  # First linear layer to reduce dimensionality
        self.relu = nn.ReLU()  # ReLU activation to add non-linearity
        self.fc2 = nn.Linear(1024, 768)  # Final linear layer to project to [B, 768]
    ...
    def forward_features(self, x, encoded_features=None):
        # B, C, T, H, W
        B, C, T, H, W = x.shape
        ...
        x = self.norm(x)  # Normalize final encoded features

        # Extract CLS token representation (which is the first element)
        cls_token_output = x[:, 0]

        # If `encoded_features` is provided, concatenate with `cls_token_output`
        if encoded_features is not None:
            encoded_features = encoded_features.to(torch.float16)
            # Mean pooling + concatenate
            encoded_features = torch.mean(encoded_features, dim=1)  # Resulting shape: [B, 1, 2, 1024]
            # Flatten the auxiliary dimensions to get [B, 2048]
            encoded_features = encoded_features.view(B, -1)  # Shape: [B, 2048]
            cls_token_output = torch.cat((cls_token_output, encoded_features), dim=1)  # Concatenate
            
            # Project to 768 dimensions
            # encoded_features = self.flatten_layer(encoded_features)  # Shape: [B, T, 2048]
            # # Use linear layers to reduce from [B, T, 2048] to [B, T, 768]
            # # First layer reduces 2048 -> 1024, with ReLU non-linearity
            # encoded_features = self.fc1(encoded_features)  # Shape: [B, T, 1024]
            # encoded_features = self.relu(encoded_features)  # Apply ReLU activation
            # # Second layer reduces 1024 -> 768
            # encoded_features = self.fc2(encoded_features)  # Shape: [B, T, 768]
            # # We need to match this with the CLS token output, which has shape [B, 768]
            # # Aggregate over the temporal frames to produce a single feature per batch
            # encoded_features = torch.mean(encoded_features, dim=1)  # Shape: [B, 768]
            # # Add element-wise to CLS token output
            # cls_token_output = cls_token_output + encoded_features  # Shape: [B, 768]
            
        return cls_token_output

    def forward(self, x, encoded_features=None):
        # Pass through the modified `forward_features`
        # print(f"Encoded Feature Shape: {encoded_features.shape}")
        x = self.forward_features(x, encoded_features)
        # print(f"Before Classifier Head Shape: {x.shape}")
        x = self.head(self.fc_dropout(x))
        return x


@register_model
def surgformer_HTA_KCA_dt_feature(pretrained=False, pretrain_path=None, **kwargs):
